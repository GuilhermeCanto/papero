...
import type { CashFlowDay } from "./finance-calculations";
...
export function TransactionsOverviewCard({
  ...
  const t = useTranslations("Dashboard.financeTransactionsOverview");
  const numbersHidden = useDashboardPrivacyStore((state) => state.numbersHidden);
  const hasDerivedData = Array.isArray(cashFlowDays) && cashFlowDays.length > 0;
  const derivedChartData = hasDerivedData
    ? cashFlowDays?.map((day) => ({
        date: day.date.toISOString(),
        expense: day.expenseCents / 100,
        income: day.incomeCents / 100 || undefined,
        timestamp: day.date.getTime(),
      }))
    : undefined;
  const activeChartData = derivedChartData ?? chartData;
  const privateChartData = activeChartData.map(({ date, timestamp }) => ({ date, timestamp }));
  const visibleChartData = numbersHidden ? privateChartData : activeChartData;
  const activeTicks =
    derivedChartData && derivedChartData.length > 0
      ? derivedChartData
          .filter((_, index) => index % Math.max(1, Math.ceil(derivedChartData.length / 7)) === 0)
          .map((day) => day.timestamp)
      : weekdayTicks;
  const activeDomain =
    derivedChartData && derivedChartData.length > 0
      ? [derivedChartData[0].timestamp, derivedChartData.at(-1)?.timestamp ?? derivedChartData[0].timestamp]
      : chartDomain;
  const chartConfig = {
    expense: {
      color: "var(--chart-4)",
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardAction>
          <Select defaultValue="weekly">
            ...
            <SelectContent>
              <SelectGroup>
                <SelectItem value="weekly">{t(chartPeriodKeys.weekly)}</SelectItem>
                <SelectItem value="monthly">{t(chartPeriodKeys.monthly)}</SelectItem>
                <SelectItem value="yearly">{t(chartPeriodKeys.yearly)}</SelectItem>
              </SelectGroup>
            </SelectContent>
          </Select>
        </CardAction>
      ...
      <CardContent>
        <ChartContainer config={chartConfig} className={cn("w-full", compact ? "h-32" : "h-50")}>
          <LineChart accessibilityLayer data={visibleChartData} margin={{ bottom: 0, left: 0, right: 0, top: 0 }}>
