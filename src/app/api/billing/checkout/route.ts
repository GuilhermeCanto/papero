...
export async function POST(request: Request) {
  if (!isDatabaseMode()) {
    return NextResponse.json({ error: "Stripe Checkout is disabled in this Papero data mode." }, { status: 404 });
  }
  ...
  try {
    const result = await billing.createCheckoutSession(request.headers, await request.json());
    ...
    return NextResponse.json({ error: "Papero could not start Stripe Checkout. Please try again." }, { status: 500 });
  }
}
